...
final class ReviewController {
    ...
    func addReview(_ req: Request) throws -> EventLoopFuture<ReviewResponse> {
        ...
        let response = ReviewResponse(id_user: dbMock?.user.id ?? 0, name_user: "\(dbMock?.user.firstname ?? "error name") \(dbMock?.user.lastname ?? "error name")", id_review: Int.random(in: 200...10000), text: body.text)
